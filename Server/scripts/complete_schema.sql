...
CREATE DATABASE property_management_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE property_management_db;

-- 1. USERS TABLE (Enhanced for Real-Time)
CREATE TABLE users (
    ...
    -- Flexible permissions and settings
    settings JSON,
    permissions JSON,
...
-- 2. USER SESSIONS (WebSocket Management)
CREATE TABLE user_sessions (
    ...
    INDEX idx_session_socket (socket_id),
    INDEX idx_session_status (connection_status),
    INDEX idx_session_last_ping (last_ping)
) ENGINE=InnoDB;

-- 3. PROPERTIES TABLE (Enhanced)
CREATE TABLE properties (
    ...
    INDEX idx_property_manager (assigned_manager_id),
    INDEX idx_property_tenant (tenant_id),
    INDEX idx_property_last_activity (last_activity),
    SPATIAL INDEX idx_property_coordinates (coordinates)
) ENGINE=InnoDB;
...
CREATE TABLE tenants (
    ...
    UNIQUE KEY unique_active_tenancy (user_id, status),
...
CREATE TABLE vendor_tasks (
    ...
    INDEX idx_task_last_activity (last_activity)
) ENGINE=InnoDB;
...
CREATE TABLE payments (
    ...
    INDEX idx_payment_paid_date (paid_date),
...
CREATE TABLE realtime_metrics (
    ...
    INDEX idx_metrics_bucket (time_bucket),
...
CREATE TABLE manager_profiles (
    ...
    INDEX idx_manager_location (location),
    INDEX idx_manager_availability (availability_status),
    INDEX idx_manager_load (current_load, capacity)
) ENGINE=InnoDB;
...
CREATE TABLE property_vendors (
    ...
    INDEX idx_pv_property (property_id),
    INDEX idx_pv_vendor (vendor_user_id),
    INDEX idx_pv_status (status),
    INDEX idx_pv_last_access (last_access)
) ENGINE=InnoDB;
...
CREATE TABLE property_managers (
    ...
    INDEX idx_pm_manager (manager_id),
...
CREATE TABLE maintenance_requests (
    ...
    INDEX idx_mr_category (category),
    INDEX idx_mr_assigned (assigned_to)
) ENGINE=InnoDB;
...
CREATE TABLE file_attachments (
    ...
    INDEX idx_file_public (is_public)
) ENGINE=InnoDB;

-- Additional Performance Indexes
CREATE INDEX idx_users_role_status_online ON users(role, status, is_online);
CREATE INDEX idx_properties_status_activity ON properties(status, last_activity DESC);
CREATE INDEX idx_tasks_status_priority_due ON vendor_tasks(status, priority, due_date);
CREATE INDEX idx_messages_conversation_created ON messages(conversation_id, created_at DESC);
CREATE INDEX idx_notifications_user_read_created ON notifications(user_id, is_read, created_at DESC);
CREATE INDEX idx_audit_events_timestamp_resource ON audit_events(event_timestamp DESC, resource_type, resource_id);

-- Partial indexes for active records
CREATE INDEX idx_active_properties ON properties(status) WHERE status IN ('LISTED', 'OCCUPIED', 'MAINTENANCE_ACTIVE');
CREATE INDEX idx_active_tasks ON vendor_tasks(status) WHERE status IN ('OPEN', 'VENDOR_ASSIGNED', 'IN_PROGRESS');
CREATE INDEX idx_online_users ON users(id) WHERE is_online = TRUE;
CREATE INDEX idx_unread_notifications ON notifications(user_id, id) WHERE is_read = FALSE;

-- Covering indexes
CREATE INDEX idx_property_details_covering ON properties(owner_id, status, title, address, last_activity);
CREATE INDEX idx_task_details_covering ON vendor_tasks(property_id, assigned_vendor_id, status, priority, due_date);
