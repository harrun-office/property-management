...
const AdminSidebar = ({ mobileOpen, setMobileOpen, setIsSidebarHovered }) => {
    ...
    const NavGroup = ({ title, children }) => (
        <div className="mb-6">
            {!isCollapsed && (
                <h3 className="px-4 mb-2 text-xs font-semibold text-[var(--ui-text-muted)] uppercase tracking-wider whitespace-nowrap">
                    {title}
                </h3>
            )}
    ...
    const content = (
        <div className="flex flex-col h-full bg-[var(--ui-bg-surface)] border-r border-[var(--ui-border-default)]">
            ...
            <div className="flex-1 overflow-y-auto py-6 px-3 custom-scrollbar">
                ...
                <NavItem to="/admin/credentials/managers" icon={<UsersIcon />}>Managers</NavItem>
                <NavItem to="/admin/credentials/vendors" icon={<BriefcaseIcon />}>Vendors</NavItem>
                <NavItem to="/admin/revenue" icon={<CashIcon />}>Hire Revenue</NavItem>

                <div className="border-t border-[var(--ui-border-default)] my-4 mx-2"></div>

                <NavItem to="/admin/settings" icon={<CogIcon />}>Settings</NavItem>
            </div>
